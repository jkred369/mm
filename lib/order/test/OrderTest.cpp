...
namespace mm
{
	...
	{
		DummyPublisher(Dispatcher& dispatcher, QueueBasedObjectPool<OrderSummaryMessage>& summaryPool, QueueBasedObjectPool<TradeMessage>& tradePool) :
			PublisherAdapter<OrderSummaryMessage> (dispatcher, summaryPool),
	...
	TEST(OrderTest, SendLiveTradeCase)
	{
		...
		// trade
		{
...

			{
				...
				const TradeMessage& trade = publisher.tradeMap[sub];
